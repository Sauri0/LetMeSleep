...
from audit_hand_deformation import HandDeformation
from verify_human_menu import action_for_rig
...
for species in SPECIES:
    ...
    for kind in ['blend','fbx']:
        ...
        for clip in source['clips']:
            name=clip['name']; action=action_for_rig(name)
            activate(rig,action)
        ...
        if species=='Human':
            ...
            blink=action_for_rig('Human_Blink')
            ...
            FACIAL.append({'format':kind,'measurement_scope':'Eye-weighted legacy blink signal; pupil only for new facial rig. Actual lid closure is audited separately.',
                           'blink_eye_height_samples':extents,
                           'closed_open_height_ratio':extents[-1][1]/extents[0][1] if len(extents)==2 else None})
            if rig.pose.bones.get('Jaw'):
                hit=action_for_rig('Human_Hit')
                ...
                for clip_name in ['Idle','Hit','Fall','Faint','Recover']:
                    action=action_for_rig('Human_'+clip_name)
            ...
            action=action_for_rig('Human_FingerCurl')
            activate(rig,action)
            start,end=map(float,action.frame_range)
            ...
            action=action_for_rig('Human_Swat')
            activate(rig,action);start,end=map(float,action.frame_range)
