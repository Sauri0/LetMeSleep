...
def clothing(mesh, tube, side, sign, cloth, piping):
    ...
    for v in leg.data.vertices:
        ...
        # Native menu audit located the six back-of-thigh intrusions on the
        # z=.70 ring (only18.3% Hips). Retain the whole upper ring with the
        # pelvis so bending the thigh cannot pull that cloth into the cushion.
        hips=ramp(v.co.z,.64,.745)
