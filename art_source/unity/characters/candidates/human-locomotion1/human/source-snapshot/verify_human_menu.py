...
def action_for_rig(name):
    matches=[a for a in bpy.data.actions if a.name.endswith(name) and any(f.data_path.startswith('pose.bones[') for f in curves(a))]
    assert len(matches)==1,(name,[a.name for a in matches])
    return matches[0]
...
def main():
    ...
    for kind in ['blend','fbx']:
        ...
        actions={name:action_for_rig(name) for name in CLIPS}
