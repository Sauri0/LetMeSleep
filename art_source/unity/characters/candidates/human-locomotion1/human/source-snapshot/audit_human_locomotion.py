...
def capture_arm_envelopes(rig, kind):
    """Functional bone data only; tool reach belongs to the imported runtime mount."""
    result = []
    for name in ['Human_Idle', 'Human_Crouch', 'Human_Swat'] + [p['clip'] for p in PROFILES]:
        ...
        if name == 'Human_Swat':
            times |= {tick / 30 for tick in range(19)}
        if name in {p['clip'] for p in PROFILES}:
            times |= {phase * duration for phase in sample_phases()}
        rows, bounds, steps, ranges, previous = [], {}, {}, {}, None
        for seconds in sorted(times):
            activate(rig, action, start + seconds * FPS)
            points = {n + '.' + side: rig.matrix_world @ rig.pose.bones[n + '.' + side].head
                      for side in ('L', 'R') for n in ('UpperArm', 'LowerArm', 'Hand', 'Socket.Grip')}
            actor = {n: [-v.x, v.z, -v.y] for n, v in points.items()}
            spans = {}
            for side in ('L', 'R'):
                for label, a, b in [('upper', 'UpperArm', 'LowerArm'), ('lower', 'LowerArm', 'Hand'),
                                     ('shoulder_hand', 'UpperArm', 'Hand'), ('shoulder_grip', 'UpperArm', 'Socket.Grip')]:
                    spans[label + '.' + side] = (points[a + '.' + side] - points[b + '.' + side]).length
            for n, point in actor.items():
                if n not in bounds:
                    bounds[n] = dict(min=point.copy(), max=point.copy())
                for axis in range(3):
                    bounds[n]['min'][axis] = min(bounds[n]['min'][axis], point[axis])
                    bounds[n]['max'][axis] = max(bounds[n]['max'][axis], point[axis])
                if previous is not None:
                    steps[n] = max(steps.get(n, 0.), (points[n] - previous[n]).length)
            for n, value in spans.items():
                ranges.setdefault(n, dict(min=value, max=value))
                ranges[n]['min'] = min(ranges[n]['min'], value)
                ranges[n]['max'] = max(ranges[n]['max'], value)
            rows.append(dict(seconds=seconds, phase=seconds / duration, actor_m=actor, spans_m=spans,
                             grip_matrices_source={side: [list(row) for row in rig.matrix_world @ rig.pose.bones['Socket.Grip.' + side].matrix]
                                                   for side in ('L', 'R')}))
            previous = points
        result.append(dict(kind=kind, clip=name, samples=len(rows), duration_seconds=duration,
                           maximum_sample_interval_seconds=max(b['seconds'] - a['seconds'] for a, b in zip(rows, rows[1:])),
                           sampled_aabb_actor_m=bounds, maximum_adjacent_sample_displacement_m=steps,
                           length_and_span_ranges_m=ranges, rows=rows))
    return result
