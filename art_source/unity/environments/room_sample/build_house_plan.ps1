...
$plan=[ordered]@{
    ...
    lobby=@{scene_id='private-lobby-v1';separate_scene=$true;bounds=@{min=@(-5,0,-4);max=@(5,3.2,4)};central_clear=@{min=@(-3,0,-2);max=@(3,3.2,2)};perimeter_min_width=1.8;spawn_clearance_radius=.4;spawns=$spawns;capacity_status='16 geometric candidates only; capacity/performance/network not certified'}
