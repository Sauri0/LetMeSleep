...
if($files.Count -ne 15){throw 'Content hash file list must match builder'}
$sha=[System.Security.Cryptography.SHA256]::Create()
$hashes=@()
try{
    foreach($map in @('house-patio-v1','private-lobby-v1')){
        $payload=$map+"`n"
        foreach($file in $files){$path=Join-Path $repository $file
        ...
        $hashes+=@{map_id=$map;content_hash=$hash}
