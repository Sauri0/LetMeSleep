...
# No project source, model or preferences are changed by this runner.
...
foreach($case in @(
    @{Name='tools-demo';Fixture='tools07_demo';ExpectedChecks=15}
)) {
    ...
    try {
        ...
        if($data.failures -ne 0 -or $data.checks -ne $case.ExpectedChecks) { throw ('Unexpected checks: '+$case.Name) }
        ...
        $capture=[ordered]@{file=$movie;exe_sha256=$actualSha;fixture=$case.Fixture;duration_s=[double]$media.format.duration;video_codec=$video.codec_name;audio_codec=$audio.codec_name;fps=30;resolution=@(1280,720);audio_replaced=$false;benchmark=$false}
...
$output=Join-Path $previewDir 'Let-me-sleep-0.7-objetos.mp4'
& ffmpeg -hide_banner -loglevel error -y -i $captures[0].file -map '0:v:0' -map '0:a:0' -c:v libx264 -threads 4 -preset medium -crf 18 -pix_fmt yuv420p -c:a aac -b:a 192k -movflags +faststart $output
