param([string]$Executable, [string]$EvidencePrefix = ('perf-rc09-'+(Get-Date -Format 'yyyyMMdd-HHmmss')))
...
foreach($case in @(@{name='two-players';count=2;shadows=-1},@{name='sixteen-players';count=16;shadows=-1},@{name='sixteen-without-shadows';count=16;shadows=0})){
    ...
    $arguments=@('--audio-driver','Dummy','--script','res://tests/performance07_live.gd','--','--map-id=house-v1-1','--natural-doors','--resolution=1080',('--population='+$case.count),('--report='+$report))
    ...
    if($data.samples -lt 30 -or $data.fps_limit -ne 0 -or $data.vsync -ne 'disabled' -or $data.rendered_map_id -ne $data.authoritative_map_id){throw 'Incomplete or mismatched performance scenario.'}
...
@{exe_sha256=$exeHash;cases=$results;scope='Local host plus bots, same generated map, 1080p, unlimited FPS. No WAN or minimum-hardware certification. Shadow-off is diagnostic only.'} | ConvertTo-Json -Depth 10 | Set-Content (Join-Path $evidence 'summary.json')
