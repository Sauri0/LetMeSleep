...
func run() -> void:
	...
	network.host_online(config, "Cancelled lifecycle check")
	await process_frame
	network.cancel_connect()
	check(await wait_until(func()->bool:return not network.online_cleanup_pending(),25.0), "cancelled SDK startup drains before retry")
	check(not network._had_session and network.invitation_text.is_empty(), "cancellation creates no usable room invitation")
