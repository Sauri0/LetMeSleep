...
$packageDir = Join-Path $outputsDir 'Let-me-sleep-0.3.0-Windows'
$zipPath = Join-Path $outputsDir 'Let-me-sleep-0.3.0-Windows.zip'
if (Test-Path -LiteralPath $zipPath) {
    $zipPath = Join-Path $outputsDir ('Let-me-sleep-0.3.0-Windows-' + (Get-Date -Format 'yyyyMMdd-HHmmss') + '.zip')
}
