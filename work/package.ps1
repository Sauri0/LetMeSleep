...
$packageDir = Join-Path $outputsDir 'Dejame-dormir-0.1.0-Windows'
$zipPath = Join-Path $outputsDir 'Dejame-dormir-0.1.0-Windows.zip'
if (Test-Path -LiteralPath $zipPath) {
    $zipPath = Join-Path $outputsDir ('Dejame-dormir-0.1.0-Windows-' + (Get-Date -Format 'yyyyMMdd-HHmmss') + '.zip')
}
