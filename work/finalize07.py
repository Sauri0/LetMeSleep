...
if '--manifest' not in sys.argv:
    ...
    demos=[read(WORK/f'release07-{name}.json') for name in ['tools-demo']]
    assert all(d['failures']==0 and d['capture']['exe_sha256'].upper()==sha(EXE) for d in demos)
    for name in ['tools-demo']: shutil.copy2(WORK/f'release07-{name}.json',EVIDENCE/f'release07-{name}.json')
    ...
    summary=dict(version=VERSION,code_commit=code,exe=artifact(EXE),checks=checks,network=network,performance=performance,demos=demos)
...
Godot 4.5.2, Windows x86_64, Compatibility/OpenGL, protocolo 7 e invitaciones DD3. Código `{code}`. EXE SHA256 `{sha(EXE)}`. Paquetes y commit final de documentación en MANIFIESTO-0.7.0.json, sin referencias circulares.
