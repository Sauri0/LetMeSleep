...
func run() -> void:
	...
	# Diagnostic: the guest has failed/cancelled several attempts while the host
	# remains in its first room. Their local attempt counters must not be shared.
	for retry in range(3):
		guest_network.join_online({}, "Pair Guest", "LMS1-invalid")
		guest_network.cancel_connect()
	check(guest_network._attempt_id != host_network._attempt_id, "independent retry counters before successful connection")
