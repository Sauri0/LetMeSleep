param([ValidateSet('native','network','performance')][string]$Suite, [string]$Executable)
...
if ($Suite -eq 'native') {
    foreach ($test in @('v07_character_rig_checks','v07_character_client_checks','selected07_actor_checks','selected07_facial_envelope_checks','facial_parts08_checks','facial_blink08_checks','customization08_checks','house07_checks','house07_liso_checks','video07_checks','doors07_client_checks','hud_input06_checks','camera_turn_checks','ui_navigation_test','throw_client07_checks','tool07_visual_checks','pickup07_support_checks','frame07_joinery_test','house07_occlusion_checks')) {
        ...
        & "$PSScriptRoot/run-native07.ps1" -Executable $exe -Name $test -GameArguments $nativeArguments
    }
    & "$PSScriptRoot/run-native07.ps1" -Executable $exe -Name occlusion-transitions -GameArguments @('--script','res://tests/house07_occlusion_checks.gd','--','--transitions')
    & "$PSScriptRoot/run-native07.ps1" -Executable $exe -Name network-throw -GameArguments @('--headless','--script','res://tests/network07_throw_checks.gd')
    & "$PSScriptRoot/run-native07.ps1" -Executable $exe -Name network08_cosmetics_checks -GameArguments @('--headless','--script','res://tests/network08_cosmetics_checks.gd')
    & "$PSScriptRoot/run-native07.ps1" -Executable $exe -Name cosmetics_catalog08_test -GameArguments @('--headless','--script','res://tests/cosmetics_catalog08_test.gd','--',('--catalog='+$PSScriptRoot+'/cosmetics-catalog08.json'))
...
if ($Suite -eq 'network') {
    & "$PSScriptRoot/run-final-network07.ps1" -Executable $exe -ExpectedExeSHA256 (Get-FileHash -LiteralPath $exe -Algorithm SHA256).Hash
}
