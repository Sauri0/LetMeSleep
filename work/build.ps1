...
if (-not $SkipTests) {
    ...
    foreach ($testName in @('modeler092_contract_test','modeler092_structure_test','environment091_stair_lighting_test','environment091_v2_finish_test','review091_online_cosmetics_contract')) {
        Invoke-CheckedHeadless -CheckName $testName -GameArguments @('--script',"res://tests/$testName.gd")
    }
