param([switch]$ResumeVerifiedR2, [switch]$ResumeVerifiedR5, [string]$EvidenceName = ('director092-build-' + (Get-Date -Format 'yyyyMMdd-HHmmss')))
...
try {
    & (Join-Path $PSScriptRoot 'build.ps1') -ResumeVerifiedR2:$ResumeVerifiedR2 -ResumeVerifiedR5:$ResumeVerifiedR5
} finally {
