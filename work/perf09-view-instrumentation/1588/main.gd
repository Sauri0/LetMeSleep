...
func _ready() -> void:
	...
	if options.has("bot"):
		...
		add_child(bot)
		return
	var client: Node = load("C:/Users/brank/Documents/Codex/2026-09-06/dejame-dormir/work/perf09-view-instrumentation/1588/client.gd").new()
	client.name = "Client"
...
func _notification(what: int) -> void:
	if what == NOTIFICATION_WM_CLOSE_REQUEST and not closing:
		...
		_cleanup_server()
		get_tree().quit(int(get_tree().get_meta("profile09_exit_code",0)))
