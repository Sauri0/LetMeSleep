...
if (-not $OutputDirectory) { $OutputDirectory = Join-Path $repo 'outputs/launcher-1.0.1' }
...
& $compiler /nologo /optimize+ /target:winexe /platform:x64 '/r:System.Windows.Forms.dll' '/r:System.Drawing.dll' @refs @branding ('/out:' + (Join-Path $OutputDirectory 'Let-me-sleep-Launcher.exe')) $core $location (Join-Path $repo 'launcher/Launcher.cs') (Join-Path $repo 'launcher/LauncherTheme.cs')
if ($LASTEXITCODE -ne 0) { throw 'Launcher compile failed' }
if (-not $SkipTests) {
    $testExe = Join-Path $repo 'work/updater-tests.exe'
    & $compiler /nologo /optimize+ /target:exe /platform:x64 @refs ('/out:' + $testExe) $core $location (Join-Path $repo 'launcher/UpdaterTests.cs')
