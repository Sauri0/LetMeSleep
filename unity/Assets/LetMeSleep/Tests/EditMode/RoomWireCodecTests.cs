    ...
    public sealed class RoomWireCodecTests
    {
        ...
        public void InvalidScalarRosterAndRuleFieldsAreRejected()
        {
            ...
            var invalid = new Dictionary<string, byte[]>
            {
                ...
                ["incomplete map version"] = Raw(map: "hf-isla-del-laguito-v", members: new[] { owner }),
                ["wrong map version"] = Raw(map: "hf-yate-a-la-deriva-v2", members: new[] { owner }),
                ["map with suffix"] = Raw(map: "hf-puerto-del-faro-v1-preview", members: new[] { owner }),
