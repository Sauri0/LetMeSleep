...
Presentación confirmó por lectura que SurfaceWalk entra con velocidad >.08 m/s y el playback se limita a 2.5x. Con .020 m/ciclo de 1 s se requiere >4x desde ese umbral: cambiar únicamente la constante no basta. La fase remota también se resincroniza desde MotionPhase de Gameplay. Presentación coordina fórmula, fase y cadencia con Gameplay; no se estiran patas ni cambian alcance/reglas desde este módulo. La captura temporal deberá medir por separado apoyo local y remoto, transición al caminar y legibilidad de la cadencia real. Sigue siendo una dependencia de integración sin fallo nativo certificado.
