...
| Pasos/carrera/escaleras | `545171d` reloj único, `3e3c57f` conexión con cuatro clips válidos, `e298930` fixes de transición/cámara/registro y `1aff7ca` importación funcional de clips. | 16 escenarios de frecuencia; Unity importó 17 clips (cuatro gaits de 2s y loop). Pendiente prueba nativa de transiciones, escucha/apoyos y gameplay. Modelo/rig/skin/morphs y 13 acciones anteriores preservados. |
...
El cuarto candidato `a82c38b` restableció tensor válido: simuló, pero falló
con gap de 5.51 cm en Leg102.L, paso18, sin reposo a10s. La revisión siguiente
evalúa subpasos en un host físico explícito, sin cambiar timestep global ni
usar una prueba distinta de la ejecución prevista. No está conectado al juego.
