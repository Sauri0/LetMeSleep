...
El coordinador debe construir `GameplayRoundConfig(..., doors: World.GetDoorDefinitions(), tools: World.GetToolDefinitions())` tanto en host como en cliente. El nuevo argumento tools es opcional al final del constructor. El mundo exige coincidencia exacta de las definiciones y los markers cargados; hasta32 unidades, IDs no repetidos, sólo flyswatter, poses finitas y rotaciones unitarias. Un mapa con markers y config sin ellos aborta el inicio, no simula herramientas fantasma.
