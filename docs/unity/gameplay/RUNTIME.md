...
- Rayo de defensa manual y barrido de mano/matamoscas. Un obstáculo cancela el resto del recorrido del golpe incluso entre ticks. El host nunca dirige la vista hacia un mosquito. `Authority.ToolId` se configura por host (`hands` inicial; `flyswatter` muestra, alias histórico `swatter`), no por mensaje cliente. El matamoscas usa mano derecha, radio de cara .085 m, distancia Grip→Impact .365 m y alcance total desde hombro 1.05 m; constantes públicas `GameplayTools` coinciden con ToolView de M1. La pose coloca la muñeca detrás del impacto, sin sumar dos veces la longitud del objeto.
