...
static func target(actor: Dictionary, yaw: float, pitch: float) -> Dictionary:
	...
	if str(actor.get("state",""))=="perched" and normal.length_squared()>.5 and forward.length_squared()>.5:
		...
		return {"basis":basis,"offset":normal*.14+forward*.05,"surface":true}
	var basis:=Basis.from_euler(Vector3(pitch,yaw,0))
	return {"basis":basis,"offset":Vector3.UP*.12-basis.z*.10,"surface":false}
