...
func run() -> void:
	...
	await _capture("ui05-host-advanced")
	...
	await _capture("ui05-customization")
	...
	ui._select_custom_role("mosquito")
	await _capture("ui05-customization-mosquito")
	await key(KEY_ESCAPE)
...
func _hud_semantics() -> void:
	...
	check("pedí ayuda" in ui._hud_hint.text and not "zona" in ui._hud_hint.text, "Rear contact asks for teammate help without revealing free zones")
	actor.role = "mosquito"
	actor.state = "flying"
	actor.lives = 1
	...
	await _capture("ui05-hud-charging")
	await key(KEY_ESCAPE)
	...
	for mode: String in ["blood","sleep"]:
		...
		if mode == "blood": await _capture("ui05-hud-stunned")
		actor.state="flying"
		...
		if mode == "blood": await _capture("ui05-hud-helping")
...
func _capture(filename: String) -> void:
	...
	var directory := ProjectSettings.globalize_path("res://../outputs/0.5-auditoria/")
