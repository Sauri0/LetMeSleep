...
func run() -> void:
	...
	var snapshot := {"config": {"mode": "sleep"}, "actors": {1: {"role": "mosquito", "alive": false, "lives": 2}}, "winner": "human"}
	ui.show_game(snapshot, {"lives": 2, "respawn_left": 3.0}, 1)
	check(ui._practice and ui._practice_banner.visible and "2 vidas" in ui._hud_state.text, "Practice state and personal sleep lives survive game transition")
