extends SceneTree
## Static diagnostic only: same scene/cameras before and after the existing
## bounded molding/railing MultiMesh probe. Does not measure gameplay FPS.
...
func _capture(stage: String, views: Array[Dictionary]) -> void:
	for view: Dictionary in views:
		...
		if root.get_texture().get_image().save_png(output+"/"+filename)!=OK:
			printerr("RENDER09_BATCH_FAIL saving "+filename)
			quit(1)
			return
...
func _run() -> void:
	if output.is_empty() or DisplayServer.get_name()=="headless":
		printerr("RENDER09_BATCH_FAIL requires native rendering and explicit output")
		quit(1)
		return
	if FileAccess.file_exists(output+"/report.json"):
		printerr("RENDER09_BATCH_FAIL evidence already exists")
		quit(1)
		return
	...
	if str(world.current_map)!=generated_id or not data.has("fingerprint"):
		printerr("RENDER09_BATCH_FAIL generated map mismatch")
		quit(1)
		return
	...
	var probe_path:=ProjectSettings.globalize_path("res://../work/static07_batch_probe.gd")
	var probe: Script=load(probe_path)
	var batches: Dictionary=probe.apply(world.map_root)
	await _capture("after",views)
	var report: Dictionary={"scope":"static environment render diagnostic; no actors, UI, simulation, gameplay FPS or visual approval","map_id":world.current_map,"fingerprint":data.fingerprint,"texture_size":root.get_texture().get_size(),"window_size":root.size,"adapter":RenderingServer.get_video_adapter_name(),"batch":batches,"records":records,"probe_sha256":FileAccess.get_sha256(probe_path),"visual_review":"pending"}
	FileAccess.open(output+"/report.json",FileAccess.WRITE).store_string(JSON.stringify(report,"\t"))
	print("RENDER09_BATCH "+JSON.stringify(report))
	world.queue_free()
	await process_frame
	await process_frame
	quit()
