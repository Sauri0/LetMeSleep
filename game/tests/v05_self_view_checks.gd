...
func check(ok: bool, message: String) -> void:
	...
	if not ok:
		failures += 1
	print("SELF_VIEW05 %s %s" % ["PASS" if ok else "FAIL",message])
...
func _run() -> void:
	...
	output_dir = ProjectSettings.globalize_path("res://../outputs/0.5-preview")
	...
	await process_frame
	await process_frame
	print("SELF_VIEW05_RESULT checks=%d failures=%d" % [checks,failures])
	quit(failures)
