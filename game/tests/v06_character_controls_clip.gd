...
func attach(zone_id: int, position: Vector3) -> bool:
	...
	simulation.actors[1].appearance = {"color":5,"accent":4,"outfit":0,"face":1,"hair":0,"accessory":3,"footwear":0}
...
func add_captions() -> void:
	...
	var stamp := Label.new()
	stamp.text = "DESARROLLO 0.6 · PRUEBA DE CONTROLES"
...
func _run() -> void:
	...
	await process_frame
	await process_frame
	print("CHARACTER_CONTROLS06_START frame=%d" % Engine.get_frames_drawn())
	...
	for movie_frame: int in range(250):
		...
		await process_frame
	var start_yaw: float = float(human.yaw)
	for movie_frame: int in range(90):
		for substep: int in range(2):
			sequence += 1
			var look_yaw: float = start_yaw+float(movie_frame*2+substep)/179.0*4.8
			simulation.submit_input(1,sequence,Vector3.RIGHT*0.3,look_yaw,-1.10,false)
			simulation.step(1.0/60.0)
			if sequence%3==0: snapshot()
		caption.text = "Giro continuo al mirar abajo · el torso acompaña la vista"
		await process_frame
	...
	print("CHARACTER_CONTROLS06_END frame=%d manual_stun=%s seconds=14.333" % [Engine.get_frames_drawn(),stunned])
