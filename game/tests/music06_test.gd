...
func _run() -> void:
	...
	music.set_context("home")
	check(music.menu_player.playing,"ordinary clear remains reusable before shutdown")
	music.shutdown()
	var accents_before := music.accents_started.duplicate()
	var ui_before := music.ui_started.duplicate()
	music.set_context("lobby")
	music.set_context("playing")
	music.accent("win")
	music.ui_cue("confirm")
	check(not music.menu_player.playing and not music.game_player.playing and not music.quiet_player.playing and not music.is_processing(),"shutdown stops playback and ignores late screen signals")
	check(music.accents_started==accents_before and music.ui_started==ui_before,"shutdown ignores late accents and UI cues")
