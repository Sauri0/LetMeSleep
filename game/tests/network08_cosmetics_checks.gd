...
func _run() -> void:
	...
	check(server.host(port)==OK,"authority starts current protocol")
	owner=peer("Owner")
	guest=peer("Guest")
	...
	check(await until(func() -> bool:return old.connection_state.get("phase","")=="failed"),"current version with old protocol7 is rejected")
	check(old.connection_state.get("code","")=="version_mismatch" and ("protocolo %d" % Net.PROTOCOL) in str(old.connection_state.get("message","")),"old checkpoint receives explicit mismatch reason")
