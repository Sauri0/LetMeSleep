...
func check(ok: bool, message: String) -> void:
	...
	if not ok:
		failures += 1
	print("CHARACTER_CLIENT06 %s %s" % ["PASS" if ok else "FAIL",message])
...
func _run() -> void:
	...
	output_dir = ProjectSettings.globalize_path("res://../outputs/0.6-arte-personajes/cliente")
	...
	await process_frame
	await process_frame
	print("CHARACTER_CLIENT06_RESULT checks=%d failures=%d" % [checks,failures])
	quit(failures)
