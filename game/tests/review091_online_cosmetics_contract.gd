...
func _run()->void:
	...
	check(host.sim.phase=="results" and guest.latest.get("phase","")=="results","authoritative results reach the guest before rematch")
	...
	var guest_waiting:Dictionary=Dictionary(lobby_packets.get("guest",{})).get("actors",{})
	check(guest_waiting.get(1,{}).get("appearance",{})==host_clean.human and guest_waiting.get(guest_id,{}).get("appearance",{})==guest_clean.human,"guest lobby receives both restored rematch appearances")
	guest.close_client()
	...
	check(not guest._online and guest_session.closed,"guest disconnect closes its fixture session and clears online state")
