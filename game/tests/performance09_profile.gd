...
class Collector:
	...
	func leave(label: String, tracked: bool, rebuilds: int=0) -> void:
		...
		if row.worst.size()<6 or inclusive>int(row.worst[-1].inclusive_us):
			row.worst.append({"inclusive_us":inclusive,"residual_us":residual,"physics_frame":span[3],"process_frame":span[4],"parent":parent,"route_rebuilds":rebuilds})
		...
		_accumulate(physics_rows,int(span[3]),label,inclusive,residual)
...
func _run() -> void:
	...
	while float(Time.get_ticks_usec()-started)/1e6<seconds+2.0:
		...
		if int(elapsed/2.5)!=last_toggle:
...
func _finish(result: Dictionary) -> void:
	...
	await process_frame
	# Let the awaiting _run coroutine release its local simulation/brain refs
	# before SceneTree teardown; generated maps retain substantial resources.
	quit.call_deferred(0 if failures.is_empty() and collector.errors.is_empty() else 1)
