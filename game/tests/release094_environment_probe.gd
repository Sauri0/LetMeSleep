...
func _capture(stage: String, views: Array[Dictionary]) -> void:
	for view: Dictionary in views:
		...
		if root.get_texture().get_image().save_png(output+"/"+filename)!=OK:
			printerr("RELEASE094_ENVIRONMENT_FAIL saving "+filename)
			quit(1)
			return
...
func _run() -> void:
	if output.is_empty() or DisplayServer.get_name()=="headless":
		printerr("RELEASE094_ENVIRONMENT_FAIL requires native rendering and explicit output")
		quit(1)
		return
	if FileAccess.file_exists(output+"/report.json"):
		printerr("RELEASE094_ENVIRONMENT_FAIL evidence already exists")
		quit(1)
		return
	...
	world.build()
	var generated_id:=Maps.default_map_id()
	...
	if str(world.current_map)!=generated_id or not data.has("fingerprint"):
		printerr("RELEASE094_ENVIRONMENT_FAIL authored map mismatch")
		quit(1)
		return
	...
	views.append({"id":"front","position":Vector3(16,10,-19),"target":Vector3(0,2,0)})
	views.append({"id":"patio","position":Vector3(-14,2,17),"target":Vector3(6,1,10)})
	views.append({"id":"stairs","position":Vector3(-10,1.6,-4.8),"target":Vector3(-10,3.5,3.2)})
	await _capture("native",views)
	var report: Dictionary={"scope":"static environment renderer diagnostic; no actors, UI, simulation, gameplay FPS or visual approval","map_id":world.current_map,"fingerprint":data.fingerprint,"texture_size":root.get_texture().get_size(),"window_size":root.size,"adapter":RenderingServer.get_video_adapter_name(),"renderer":RenderingServer.get_current_rendering_method(),"rendering_driver":RenderingServer.get_current_rendering_driver_name(),"records":records,"visual_review":"pending","fixture_sha256":FileAccess.get_sha256("res://tests/release094_environment_probe.gd")}
	...
	print("RELEASE094_ENVIRONMENT "+JSON.stringify(report))
	world.queue_free()
	await process_frame
	await process_frame
	quit()
